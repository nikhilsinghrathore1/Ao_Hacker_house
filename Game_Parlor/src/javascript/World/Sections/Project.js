...
export default class Project {
    ...
    setFloor() {
        ...
        if (this.distinctions) {
            for (const _distinction of this.distinctions) {
                ...
                this.objects.add({
                    ...
                    rotation: new THREE.Euler(0, 0, 0),
                    duplicated: true,
                    shadow: { sizeX: 0, sizeY: 0, offsetZ: -0.1, alpha: 0.5 },
                    mass: 0.1,
                    soundName: 'woodHit'
                });
            }
        ...
        this.floor.area.on('interact', async () => {
            console.log("clicked; will do Ao stuff here");
            const message = `Project "${this.name}" clicked at ${new Date().toISOString()}`;
            ...
            try {
                ...
                let transaction = await arweave.createTransaction({ data: message }, wallet);
                transaction.addTag('Content-Type', 'text/plain');
                transaction.addTag('Project-Name', this.name);

                await arweave.transactions.sign(transaction, wallet);
                let response = await arweave.transactions.post(transaction);

                if (response.status === 200) {
                    console.log("Message successfully sent to Arweave:", message);
                } else {
                    console.error("Failed to send message to Arweave:", response.statusText);
                }
            } catch (error) {
                console.error("Error sending message to Arweave:", error);
            }

            // Open the link as before
